...
long long int leftSide,rightSide,mini;
        ...
        for(long long int i=2;i<=number;i++){
            for(leftSide=0;leftSide<number;leftSide++){
                ...
                for(long long int k = leftSide+1;k<rightSide;k++){
                    mini = cut[leftSide][k] + cut[k][rightSide]+ a[rightSide] - a[leftSide];
                    
                    if (cut[leftSide][rightSide] > mini)
                    {
                        cut[leftSide][rightSide] = mini;
                    }
                }
